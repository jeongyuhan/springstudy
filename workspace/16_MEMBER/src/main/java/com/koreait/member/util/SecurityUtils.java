...
public class SecurityUtils {
	...
	/*
	 -- 아래 코드를 Java 사용 = sha256()
	 				commons-codec사용 = base64() 로 바뀌는 것이다.
			// pw의 암호화
			// Java : java.security
			// MessageDigest;
			...
			// md.digest(); // 암호화된 바이트 배열 (DB에 넣기위해선 String으로 바꿔줘야한다.)
			StringBuilder sb = new StringBuilder();
			for(byte b : md.digest()) {
				sb.append(String.format("%02x", b)); // byte b를 16진수 표기법으로 변환
			}
			String encodedPw = sb.toString();
			System.out.println("암호화 : " + encodedPw);
			// 암호화 : 0ffe1abd1a08215353c233d6e009613e95eec4253832a761af28ff37ac5a150c 이 결과를 DB에 저장하는 것이다.
			...
			StringBuilder sb = new StringBuilder();
			for(byte b : Base64.encodeBase64(pw.getBytes())) {
				sb.append(String.format("%02x", b));
			}
			String encodedPw = sb.toString();
			// System.out.println("암호화 : " + encodedPw); // 암호화 : 4d5445784d513d3d
	*/
