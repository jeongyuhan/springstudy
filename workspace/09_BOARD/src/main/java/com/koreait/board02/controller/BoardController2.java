...
import com.koreait.board02.config.BeanConfiguratuon;
...
public class BoardController2 {
	...
	// field
	private AbstractApplicationContext ctx = new AnnotationConfigApplicationContext(BeanConfiguratuon.class);
	...
	public String selectBoardByNo(@RequestParam("no") long no,
		...
		SelectBoardByNoCommand selectBoardByNoCommand = ctx.getBean("selectBoardByNoCommand", SelectBoardByNoCommand.class);
	...
	public String updateBoard(HttpServletRequest request, // update.jsp에서 전달한 파라미터
		...
		UpdateBoardCommand updateBoardCommand = ctx.getBean("updateBoardCommand", UpdateBoardCommand.class);
	...
	public String deleteBoard(@RequestParam("no") long no, // 삭제할 게시글의 no
		...
		DeleteBoardCommand deleteBoardCommand = ctx.getBean("deleteBoardCommand", DeleteBoardCommand.class);
	...
	public String insertBoard(Board board, Model model) {
		...
		InsertBoardCommand insertBoardCommand = ctx.getBean("insertBoardCommand", InsertBoardCommand.class);
